...
if '$TYPE' == 'monthly':
    ...
    if match:
        if \"href: 'images/monthly/$MONTH_YEAR.html'\" not in content:
            new_entry = \"\n                    { label: '$MONTH_SHORT', href: 'images/monthly/$MONTH_YEAR.html' },\"
...
EOF
    ...
    local js_base=".."
    if [[ "$OUT_HTML" == images/monthly/* ]]; then
        js_base="../.."
    fi
...
process_gallery "monthly" "$INCOMING_MONTHLY" "images/monthly/$MONTH_YEAR" "images/monthly/${MONTH_YEAR}.html" "$MONTH_LABEL photos" "../../styles/style.css"
